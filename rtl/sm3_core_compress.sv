...
// Engineer: 
// 
// Create Date: 2025/08/30 13:37:29
// Design Name: 
// Module Name: sm3_core_compress
// Project Name: 
// Target Devices: 
// Tool Versions: 
// Description: SM3核心压缩函数模块。
//              该模块实例化了消息扩展模块(sm3_core_word_extand)，
//              并执行64轮压缩迭代，最终生成一个256位的哈希值。
//              采用深度流水线设计以满足高时钟频率要求。
// 
// Dependencies: sm3_core_word_extand.sv
// 
// Revision:
...
// 
//////////////////////////////////////////////////////////////////////////////////
module sm3_core_compress(
    input logic                     aclk,
    input logic                     rst_n,

    input logic                     in_valid,       // 开始压缩信号                    
    input logic [0:7][31:0]         iv,             // 初始向量(IV)或上一轮的哈希值          
    input logic [0:15][31:0]        word_in,        // 每一次压缩所用的512位字（未拓展）        
    
    output logic                    out_valid,      // 压缩完成信号                    
    output logic [0:7][31:0]        v_out           // 压缩输出 (256位哈希值)            
);
    // 延迟和暂存寄存器
